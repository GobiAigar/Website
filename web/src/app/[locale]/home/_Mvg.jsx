	...
	const animateContent = (newIndex) => {
		...
		const tl = gsap.timeline({
			onComplete: () => {
				...
				gsap.fromTo(elements, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 0.6, ease: "power2.out" });
			}
		});
	...
	return (
		<Box>
			...
			>
				...
				<Typography ref={titleRef} variant="h4" sx={{ willChange: "transform, opacity" }}>
					{lang === "mn" ? datas[selected]?.mntitle : datas[selected]?.entitle}
				</Typography>

				<Typography ref={descriptionRef} variant="body1" width={"50%"} sx={{ willChange: "transform, opacity" }}>
					{lang === "mn" ? datas[selected]?.mndescription : datas[selected]?.endescription}
				</Typography>

				<Box position="absolute" bottom="5%" display="flex" alignItems="center" justifyContent="center">
